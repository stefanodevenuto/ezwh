...
class RestockOrderDAO extends AppDAO{        
    async getAllRestockOrders() {
        const query = 'SELECT RO.id AS id, RO.issueDate, RO.state, RO.supplierId, RO.deliveryDate, S.id as SKUId, S.description, S.price, ROI.qty\
            FROM restockOrder RO \
            JOIN restockOrder_sku ROI ON RO.id = ROI.restockOrderId \
    ...
    async createRestockOrder(issueDate, supplierId, state, products) {
        ...
        const query_association = 'INSERT INTO restockOrder_sku(skuId, restockOrderId, qty) VALUES (?, ?, ?)';
        ...
        for (let product of products)
            await this.run(query_association, [product.sku.id, id, product.qty]);
